...
<html lang="en">
<head>
  <title>Board of Practical Training | Journal Voucher</title>
  <meta charset="utf-8">
...
<table class="ledger" style="width:100%;border: 1px solid #000;">
    ...
    <td>{{ $party_detail->account_name }}({{ $party_detail->gl_account_head }})</td>
    @if(($party_detail->vouchertype != 'received'))
    <td>{{ $party_detail->narration }}</td>
    @else
    <td>{{ $party_detail->remarks }}</td>
    @endif
    @if((!empty($party_detail->vouchertype)) && $party_detail->payment_release_date == '0000-00-00')
    <td>{{ \Carbon\Carbon::parse($party_detail->voucher_date)->format('d/m/Y') }}</td>
    <td></td>
    <td class="right">{{ $party_detail->amount }}</td>
    @elseif((!empty($party_detail->vouchertype)) && $party_detail->voucher_date == '0000-00-00')
    <td>{{ \Carbon\Carbon::parse($party_detail->payment_release_date)->format('d/m/Y') }}</td>
    <td class="right">{{ $party_detail->amount }}</td>
    <td></td>
    @endif
    </tr>
	...
	<tr class="3">
		...
		<td class="right" style="border-top:1px dashed #000;padding:10px 0;">{{$debit_amt}}</td>
		<td class="right" style="border-top:1px dashed #000;padding:10px 0;">{{$credit_amt}}</td>
	</tr>
</table>
