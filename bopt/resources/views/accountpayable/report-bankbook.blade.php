...
<title>Board of Practical Training (Eastern Region) | Bank Book</title>
</head>
...
<thead>
	<tr>
		<th><h1 style="margin:0;font-size:25px;">BOARD OF PRACTICAL TRAINING (EASTERN REGION)</h1>
			...
			<h4 style="margin:0;"><?php if(!empty($payment_dtl[0]->bank_id)){ echo strtoupper($payment_dtl[0]->bank_id); } ?></h4>
...
<table style="width:100%;font-family:cambria;margin-top:20px;">
	...
	<tbody>
		...
		<?php $total_expense=0; $total_income = 0;
		if(!empty($payment_dtl)){foreach($payment_dtl as $payment){ ?>
		<tr>
            <?php if($payment->voucher_type == 'received') { ?>
                <td style="vertical-align:top;text-align:center;padding:5px;font-size:12px;"><?php  echo date("d-m-Y", strtotime($payment->payment_rcv_date)); ?></td>
                <td style="width:230px;vertical-align:top;padding:5px;font-size:12px;"><?php echo $payment->remarks;  ?></td>
		...
		<tr>
			...
			<td style="font-size:12px;text-align:center;padding-top: 10px;" colspan="5"></td>
			<td style="font-size:12px;text-align:right;padding-top:10px;border-top: 2px solid #000;"><?php echo $total_expense;?></td>
			<td style="font-size:12px;text-align:right;padding-top:10px;border-top: 2px solid #000;"></td>
		</tr>
		<tr>
			<td style="font-size:12px;text-align:left;padding-top: 10px;" colspan="5"></td>
			<td style="font-size:12px;text-align:right;padding-top:10px;"></td>
			<td style="font-size:12px;text-align:right;padding-top:10px;"><?php echo $closing_balance;?></td>
		</tr>
		<tr>
