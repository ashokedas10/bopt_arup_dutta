                    ...
                    <div class="main-card">
                        <div class="card">
                            <div class="card-header">
                                <strong>Add New Branch Wise Allowance</strong>
                            </div>
                            
								<div class="card-body card-block">
                                <form action="{{ url('pis/add-new-branch-allowance') }}" method="post" enctype="multipart/form-data" class="form-horizontal">
								   ...
								   <input type="hidden" name="_token" value="{{ csrf_token() }}">
									<div class="row form-group">
											...
											<div class="col col-md-3"><label for="file-multiple-input" class=" form-control-label">Allowance Amount</label>
												<input type="text" class="form-control" id="allowance_amount" name="allowance_amount">
												@if ($errors->has('allowance_amount'))
													<div class="error" style="color:red;">{{ $errors->first('allowance_amount') }}</div>
												@endif
											</div>
											<div class="col col-md-3"><label for="file-multiple-input" class=" form-control-label">Valid To</label>
												<input type="date" class="form-control" id="valid_to" name="valid_to">
												@if ($errors->has('valid_to'))
													<div class="error" style="color:red;">{{ $errors->first('valid_to') }}</div>
												@endif
											</div>
											<div class="col col-md-3"><label for="file-multiple-input" class=" form-control-label">Valid From</label>
