                ...
                <div class="row">
                    <div class="main-card">
                        <div class="card">
                            ...
                            <strong>Received Entry</strong>
                            </div>
                            <div class="card-body card-block">
                            <form action="{{ url('receiveable/payment') }}" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                <input type="hidden" name="id" value="" />

                            <div class="row form-group">
                              <div class="col-md-4">
                                <label for="text-input" class="form-control-label">Received Id </label>
...
<script>
    ...
    function getVoucherDtl(){
        ...
        $.ajax({
            type:'GET',
            url:'{{url('receiveable/payment')}}/'+voucher_no,
...
$('#voucher_type').val(response.payments.voucher_type);
                ...
                $('#bill_date').val(response.payments.created_at);
    ...
    function getPaymentMode(val){
        ...
        if(payment_mode=='cheque'){
            ...
            $('#payment_bank_id'+val).attr('readonly',false);
            $('#bank_branch_id'+val).attr('readonly',false);

            $('#receive_id'+val).attr('readonly',true);

        }else if(payment_mode=='cash'){

 $('#cheque_draft'+val).attr('readonly',true);
            $('#cheque_date'+val).attr('readonly',true);
            $('#payment_bank_id'+val).attr('readonly',true);
            $('#bank_branch_id'+val).attr('readonly',true);
            $('#receive_id'+val).attr('readonly',false);
