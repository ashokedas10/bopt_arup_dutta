            ...
            <div class="animated fadeIn">
                ...
                <div class="row">
                  <?php //echo "<pre>" ; print_r($tour_apply->duration); exit; ?>
                    <div class="main-card">
                        <div class="card">
                            <div class="card-header"><strong class="card-title">Apply for GPF Loan</strong></div>
                            <div class="card-body card-block">
                                <form action="{{ url('loan/gpf-loan-apply') }}" method="post" enctype="multipart/form-data" style="padding:2% 5%;">

                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="hidden" name="closing_balance" id="closing_balance" value="{{ $fund_check->closing_balance }}">
									...
									<div class="lv-due" style="border:none;">
										...
										<input type="text" class="form-control" name="loan_amt" id="loan_amt" value="" onchange="check_fund();" required>
										@if ($errors->has('loan_amt'))
											<div class="error" style="color:red;">{{ $errors->first('loan_amt') }}</div>
										@endif
                                        </div>
                                        <div class="col-md-4">
                                            <label for="email-input" class=" form-control-label">Purpose of Loan <span>(*)</span></label>
...
function check_fund()
{
    ...
    if(parseFloat(closing_balance) < parseFloat(loan_amt))
    {
        ...
        $("#loan_amt").val("");
        $("#from_date").val();
        wait(1000).then(() => window.location='{{ url('loan/gpf-loan-apply') }}'); // 'Hello!
