...
<form role="form" method="post" action="{{ url('dak/editfinal') }}" enctype="multipart/form-data">
               
            <input type="hidden" name="_token" value="{{ csrf_token() }}"> 
              <input type="hidden" name="final_id" value="<?php if(!empty($final_detail->final_id)){ echo $final_detail->final_id; } ?>" />
              <div class="row form-group">

                <?php if(empty($final_detail->final_id)){ ?>
                <div class="col-md-3">
                    <label for="text-input" class="form-control-label">Select Receipts</label>
                  
                    <select class="form-control" name="dak_receipt_detail_id" id="dak_receipt_detail_id" <?php if(!empty($final_detail->id)){ ?> disabled <?php } ?>  onchange="populateData()" >
                    <option>Select Dak</option>
                    <?php foreach($receipt_details_code as $receipt){ ?>
                      <option  value="<?php echo $receipt->id; ?>" <?php if(!empty($final_detail->dak_receipt_detail_id)){ if($receipt->id == $final_detail->dak_receipt_detail_id) { echo "selected"; } } ?>><?php echo $receipt->dairy_no; ?></option>
                    <?php } ?> 
                    </select>
                  </div>
            ...
            <?php }else{?>
              <div class="col-md-3">
                <input type="hidden" name="hidden_recept_id" value="<?php if(!empty($final_detail->dak_receipt_detail_id)){ echo $final_detail->dak_receipt_detail_id; } ?>" />
              </div>
            <?php } ?>
         ...
         <option value="West Bengal" <?php if(!empty($final_detail->state)){ if("West Bengal"== $final_detail->state) { echo "selected"; } } ?>>West Bengal</option>
          ...
          <div class="col-md-3">
            ...
            <div class="col-md-3">
              ...
              <div class="radio-inline">
                <label><input type="radio" name="status" value="open" <?php if(!empty($final_detail->status)) {if($final_detail->status == "open"){ echo "checked";}}  ?> checked>Open</label>
              </div>
               <div class="radio-inline">
                <label><input type="radio" name="status" value="closed" <?php if(!empty($final_detail->status)) {if($final_detail->status == "closed"){ echo "checked";}}  ?> >Close</label>
              </div>
            </div>
          ...
          <div class="col-md-3">
            ...
            <input type="text" class="form-control" value="" name="closing_remarks" id="closing_remarks" readonly>
          </div>
        
          <div class="col-md-3">
            <label>Closing Date</label>
            <input type="date" class="form-control" value="" name="closing_date" id="closing_date" readonly>
          </div>
          </div>
        

          
        <input type="submit" value="Update Receipt" name="submit" class="btn btn-danger btn-sm">
        </form>
				</div>
      </div>
...
<script>
  ...
  $("input[type='radio']").click(function(){
      ...
      if(radioValue=='closed'){
        $('#closing_date').attr("readonly", false);
        $('#closing_remarks').attr("readonly", false);
      }
    });
