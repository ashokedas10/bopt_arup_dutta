...
<table class="comp-det" style="width:100%;">
		<tr>
			...
			<td>
				...
				<h4>Schedule 12 - Interest Earned</h4>
				</div>
...
<table border="1" style="width:100%;border-collapse:collapse;">
	<thead>
		...
		<tbody>
                ...
                @if($schedule_12['account_name'] == $schedule_expense['account_name'])
            ...
            <tr>
                <td style="padding:3px;">{{ $sub_act['sub_account_name'] }}({{ $sub_act['coa_code'] }})</td>
                <td class="right" style="padding:3px;">{{$total_amt}}</td>
                <td class="right" style="padding:3px;">-</td>
            </tr>
            @endforeach
            @else
            ...
            @endif
            @else
            @foreach($schedule_12['sub_coa_name'] as $coa_dtl)
            @php $coa_detail[] = array_unqiue($coa_dtl); @endphp
            @if($coa_detail['sub_head'] == '')
            <tr>
                <td style="padding:3px;"> {{ $schedule_12['account_name'] }}</td>
                <td class="right" style="padding:3px;">{{$total_amt}}</td>
                <td class="right" style="padding:3px;">-</td>
            </tr>
            @else
            <tr>
                <td style="padding:3px;"> {{ $coa_detail['account_name'] }}</td>
                <td class="right" style="padding:3px;">-</td>
                <td class="right" style="padding:3px;">-</td>
            </tr>
            <tr>
                <td style="padding:3px;"> {{ $coa_dtl['sub_head'] }}</td>
                <td class="right" style="padding:3px;">{{$total_amt}}</td>
                <td class="right" style="padding:3px;">-</td>
            </tr>
            @endif
